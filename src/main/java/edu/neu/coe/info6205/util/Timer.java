...
public class Timer {
    ...
    public <T, U> double repeat(int n, Supplier<T> supplier, Function<T, U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
        ...
        T supplier1 = supplier.get();
        T preFuncOutput = null;
        pause();

        for (int i=0; i<n; i++) {
            if (preFunction != null) {
                preFuncOutput = preFunction.apply(supplier1);
            }
            resume(); // ticks-=getClock(), running=true
            U funcOutput = function.apply(preFuncOutput);
            pauseAndLap(); // laps++, ticks+=getClock(), running=false
            if (postFunction != null) {
                postFunction.accept(funcOutput);
            }
        }
